...
public:
        ...
        // {}
        Receiver(uint8_t pin) : Sensor(pin), filter(0.3, analogRead(m_pin)) {}
        ...
        //                 // 5 is an arbitrary threshold that seems to work
        //                 well
        //                 // 4 is an arbitrary threshold that seems to work
        //                 well
        //                 // return absVal(a - b) < 5.0;
        //                 return absVal(a - b) < 2.5;
        //         };
        //         // Serial.print(millis());
        //         // Serial.print(' ');
        ...
        //                 // }
        //                 if (currStatus == Status::receiving || currStatus ==
        //                 Status::start) {
        ...
        Status getStatus() {
                static Status toReturn = Status::none;
                static int    prev     = filter.current();
                // If message has just started, set to receiving
                if (toReturn == Status::start)
                        toReturn = Status::receiving;
                // If message has just ended, set to none
        ...
        void update() override {
                ...
                if (millis() - lastUpdated > samplePeriod) {
                        ...
                        else if (currStatus != Status::none && currStatus != Status::ending) {
                                count += millis() - lastUpdated;
                                // Serial.println(count);
                        }
...
private:
        ...
        // long unsigned                                  samplePeriod = 3;  //
        // ms long unsigned                                  samplePeriod = 5;
        // // ms
        long unsigned samplePeriod = 0;  // ms
